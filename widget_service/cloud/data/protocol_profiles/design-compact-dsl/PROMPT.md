...
**2x2 单业务多字段生成前置约束**：`/data` 下只有一个一级业务根时始终是单业务；同一根内的名称、状态及多个指标不得按字段数拆成 S4。禁止生成单个或残缺的 `134×63vp` S4 内容背板，使用全宽单业务信息流。用户明确要求展示且不用于动作参数的 1-3 个不同字段必须全部保留并各显示一次；“重点、优先、主要”只调整字段顺序和视觉主次，不得删除其余明确字段。重点字段放在前面。为适配 `150×150vp` 画布，普通 S1/S2 的全部信息区统一使用以下三类密度：①单一数字焦点含 `30fp/38fp` 大数字时，数字与真实单位合计为一行，其余字段最多合并成一条 `12fp/400` 辅助信息行并以 ` | ` 分隔；②单一文字焦点最多一行突出文字和两行 `12fp/400` 辅助信息；③两个及以上同级并行指标不得制造大数字焦点，每项压成一条普通字号完整信息，最多三行。标题和 action 不计入信息行；V01、V06、S3、S4、含 Progress 的视觉亚型及 2x4 W8/W10 小背板不适用本规则。2x4 W9 的左右两个 `138×134vp` 大背板必须分别把内部业务标题和 action 排除后独立执行同一密度规则，不能因位于 2x4 内而恢复额外信息行或多个突出焦点。普通单业务内容区不生成 Image，不得为使用候选素材套用 S4 小背板。

**单业务并行指标生成前置约束**：大数字 `value_row` 只能包含纯数字 Text 和紧邻的真实单位 Text，禁止在该 Row 内放入标签、方向、状态、名称、说明或其它字段；这些信息必须放在主值下一行。同一业务对象内存在最高/最低、当前/目标、已用/剩余、左/右等两个及以上同级量化指标时，所有指标必须上下排列，禁止用 Row 拆成左右两列或左右两个指标组；同时禁止选中其中一项使用 `30fp/38fp` hero、数值与标签混合字号 Row 以及多个大数字 `value_row`，每个指标必须压成一个完整的 `12fp/400` 单行 Text，按“短标签 + 数值 + 单位”展示，并保持相同字号与字重。此规则适用于 2x2 普通 S1/S2，也适用于 2x4 W9 的每个独立大背板；不适用于 V01、V06、S3、S4 和 2x4 W8/W10 小背板。
...
**2x4 双业务生成前置约束**：必须先按业务对象实例合并字段，再统计数据块；同一耳机对象的名称、连接状态、耳机仓电量和充电状态固定算一个数据块，禁止拆成“耳机连接”和“耳机仓电量”两个数据块。最终展示恰好两个业务数据块时立即锁定 W9，root 必须是 Row、`padding:8`，直接且只能包含左右两个 `138×134vp` 大内容背板，间距 `8vp`；禁止复用 2x2 S4，禁止 `root Stack -> content Column`，禁止两个全宽背板上下排列，禁止任何其它上下双业务布局。禁止卡级公共标题、跨业务公共内容行、卡级 `actions/action_area` 和背板外按钮。每个业务的标题、数据和至多一个按钮必须全部放在自己的背板内，action 数量不增加、拆分或重排业务数据块，也不得为了容纳 action 改走 W10。W9 大背板带动作时直接子节点固定为 `[content, action]`，`content` 必须 `layoutWeight:1`，action 必须是最后一项并固定 `114×36vp`。纯文字 action 使用 Button；图文 action 使用 `Row -> [Image, Text]`，Row 必须显式写 `itemMargin:8`、左右 `padding:8`、`justifyContent:"center"`、`alignItems:"center"`，Image 固定 `20×20vp`。`content` 最多三行视觉信息；数字与单位拆成两个 Text 时仍属于同一行。辅助字段过多时先用 ` | ` 合并同级短字段，仍超出则删除低优先级可选字段。

**2x4 四数据生成前置约束**：最终展示恰好四个业务数据块时立即锁定 W8，无论用户是否提到标题都禁止卡级标题、CardHeader、header 及标题空槽；root 固定 `padding:8`，`284×134vp` 安全内容区必须完整用于 `2×2` 网格，固定为四个 `138×63vp` 小内容背板，横纵间距均为 `8vp`。root/全尺寸 grid 之外不得增加 content 包装或其它一级区域，不得为标题压缩背板尺寸。
...
**2x2 双按钮前置约束**：先按用户意图选择动作，再决定骨架。单业务最终保留两个明确动作时，S3 优先于普通 S2 数值/状态布局；候选中恰有两个事件不等于必须保留两个。root 只能是 Column -> [header_area, action_area]，禁止 CardHeader + content_area + action_area。信息组 40vp + 根间距 8vp + 两个 36vp 按钮及内部间距 6vp = 126vp。第一行对象与主值合并为 14fp/700 文本（如“手机电量 68%”），第二行 12fp/400 状态紧随其下；不放标题图标，不用大数字，不省略用户要求的主值。两个动作保持原候选绑定，不能为腾空间删除用户明确动作。
...
- 生成前先写出一句内部判断：“用户打开卡片首先要读到什么”。全卡只能有一个第一焦点；标题、主值、状态和按钮不能同时争夺第一焦点。
- 主焦点优先使用“位置 + 尺度 + 留白”建立：主值或核心状态放在所属主区域第一行/视觉中心，周围保留连续留白；辅助字段必须贴近对应主值，不得平均散落在四角。
- 主辅字号必须形成可感知差异：主焦点使用批准档位中最高且安全的一档，辅助信息至少低一档，弱提示/单位使用 12fp。除 S3 固定 14fp 外，不要让整张卡片的正文全部落在同一字号。
- 同组并列指标需要统一字号；只有跨层级内容才拉开字号差异。不要通过把所有文字都加粗或放大来制造重点。
- 纯数字主值优先拆成“数值 Text + 真实单位 Text”；格式化字符串只有在满足受控例外且确实是第一焦点时才放大。主值放不下时，先删弱辅助字段或改布局，再统一降档，不得压扁所有文字。
- 2x4 在固定外框和数据块路由不变的前提下，允许在每个骨架内部选择不同的内容变体：`value-led`（主值居中/靠上）、`status-led`（状态句为主）、`event-led`（事项标题与时间为主）、`action-led`（动作是目标）和 `dense-summary`（多指标紧凑汇总）。变体只改变内部顺序、对齐、留白和字段取舍，不新增一级区域、不跨骨架拼接。
- 2x4 多数据卡不要机械复制“标题 + 两行 14/12fp + 按钮”。每个分区先确定自己的主焦点：主值分区突出数值，状态分区突出状态句，动作分区突出动作；同一卡允许不同分区采用不同变体，但同一分区内仍遵守字号和对齐一致性。
- **重心硬约束**：生成前先判断卡片属于“单焦点”还是“并列焦点”。没有用户明确要求并列比较时，全卡默认只保留一个第一焦点；其它区域通过至少两项差异（字号、位置、连续留白、颜色明度、占用面积）降为支撑层。标题不能和主值同时使用最高强调，按钮不能通过更高饱和度抢过主值。
- **2x2 密度约束**：单业务且没有 CTA 时，主信息组优先垂直居中或上方居中，辅助字段紧贴主值；不得把标题、主值和辅助信息全部贴在顶部后留下无意义的中下部空白。存在 CTA 时，内容区使用 `layoutWeight` 吸收剩余空间，主信息组保持连续，CTA 沉底但不成为第一焦点。
- **2x4 分区差异约束**：W9/W10 默认采用“一主一辅”重量关系；主分区可使用 24/30fp 主读数或 16/18fp 主状态，辅分区优先使用 14/18fp 主值和 12fp 支撑信息。只有用户明确要求两项等权比较时才让两个分区使用相同主字号；W8 等权网格也要通过字号或颜色建立一个轻量主次，不得所有单元格完全同权。
- **视觉节奏约束**：同一分区最多保留一个主表面和一个辅助表面；连续信息组的相邻行间距使用 2–8vp，分区之间使用 8–12vp。不要使用空容器、重复背板、重复标签或弱装饰填充剩余高度。内容较少时，稳定留白优于增加无意义字段。
...
- 真实 userQuery 可能只说“做一张天气/日程/设备卡片”，不使用“按钮、打开、查看”等专业动作词。此时如果候选中存在与当前主业务严格匹配且无副作用的详情入口，允许自动保留一个 root 或所属分区点击入口；该入口不额外占用视觉区域、不生成假按钮、不改变主焦点。设置、导航、拨号、播放、入会、删除、切换等副作用动作仍需用户语义支持。
...
- `assetCandidates` 只规定允许使用的素材范围，仅在语义精确匹配后按 8.3 节判断是否使用；不得为了使用候选素材而新增内容区或图标槽。对 `asset.*.png` 彩色位图，只要其 `description` 明确匹配当前单一主业务并允许主题图使用即可考虑；不再以透明背景作为前置排除条件，最多使用一枚并保留原色，优先放标准 2x2 S2 CardHeader 现有右上 20vp 位。2x4、S1/S3/S4、正文、按钮、环中心和融球仍不主动使用该类 PNG。
...
- `2x2` 图标区域强互斥。对普通素材仍按「内容区 > 按钮区 > 标题区」取舍；但符合上一条全部条件的彩色 PNG 是标题区的受控例外：若当前为单业务 S2、没有内容区/按钮区图标、且 CardHeader 标题稳定，则优先放入 CardHeader 右上现有 20vp 位，不改变骨架、不新增字段、不调整图标大小。其他 PNG 一律省略。无图标标题不写 CardHeader.icon，无图标动作从布局前采用纯文字并保留动作，不生成 icon-round、图文按钮或空图标槽。
...
```genui
["root","Column",{"width":"matchParent","height":"matchParent","padding":12,"borderRadius":20,"clip":true,"alignItems":"center","justifyContent":"start","linearGradient":{"direction":"RightBottom","colors":[["#FFCBDDFE",0],["#FFF1F6FE",1]]}},["header","main"]]
["header","CardHeader",{"title":"卡片标题","fontColor":"#FF1F4799"}]
["main","Text",{"content":"主要内容","fontSize":16,"fontColor":"#FF1F4799","maxLines":1}]
["/state/ready",true]
```
...
1. `2x2` 和 `2x4` 的普通 root 使用 `padding:12`，可用高度固定为 `126vp`，即 `150 - 12 - 12`；S4、W8、W9、W10 使用 `padding:8`，可用高度固定为 `134vp`。不得把 root 外部高度 `150vp` 当作内容高度。
...
5. `layoutWeight`、`flexShrink`、`clip` 和分布式对齐都不能抵消已经算出的固定高度。`H_required` 超过当前 root 的 `126vp` 或 `134vp` 有效高度时，必须删除弱区域、合并标题、降低区域固定高度或改选更简单骨架。

以下两种结构无论截图是否暂时可见都必须判定失败：

- `20 + 59 + 59 + 8 × 2 = 154 > 134`：标题行与 S4 纵堆双 `59vp` 区域不可共存；删除总标题，保留 `59 + 8 + 59 = 126`。
...
- 小内容蒙版统一指 2x2 S4 的 `134×63vp` 分区和 2x4 W8/W10 的 `138×63vp` 分区，只允许两个单行 Text：第一行对象名与主数据使用 `14fp/700`、内容色 100%，第二行辅助数据固定 `12fp/400`、第十二节必要支撑文字色，不再增加独立业务标题行；两个 Text 都必须显式设置 `maxLines:1`。需要突出量化主值时，第一行优先展示主值，但同类对象分区必须允许“对象名＋主值”；对象名不得因数字优先而省略。蒙版内 1 项数据时只输出第一行，2 项时分别放第一、二行；有语义精确匹配且状态安全的候选素材时，1-2 项使用一个右侧图标，无合法素材时不得伪造或留空槽。2x2 S4 不得根据文字字符数、是否超过 6 个字、占一行还是两行来删除、移动或缩放右侧图标；使用固定文字列，动态绑定保持单行裁切，不得换行形成第三行。2x4 W8/W10 必要对象名与主值放不下时仍可删除右侧视觉。3 项时不使用任何右侧视觉，第一行放主数据，第二行将两个辅助数据写成单个 Text 并以 ` | ` 分隔；超过 3 项时按“用户明确要求 > 主数据 > 状态 > 其他辅助”只保留前三项，其余删除。
...
选择骨架前必须先按最终展示数据识别独立业务对象：同一语义实体下的名称、电量、状态等多个字段始终只算 1 个对象，`eventCandidates`、动作数量和组件数量均不得增加对象数；不同业务实体或同类业务中的不同对象实例（如两座城市）才分别计数。只有恰好 2 个独立对象时才允许 S4，且必须完整生成上下两个 `134×63vp` 分区；只有 1 个对象时必须选择 S1/S2/S3，绝对禁止生成单个 `134×63vp` 内容背板或其它残缺 S4。单对象加两个明确动作必须选择 S3，不得把该对象的字段拆成两个 S4 分区。
...
- region：默认使用 `CardHeader 20vp` 恒高 + `content_area`（layoutWeight:1）+ `action_area` 底部锚定（36vp 胶囊）；无动作时 `action_area` 换成 `bottom_area`（一组全宽支撑信息）。2x2 单倒计时带一个动作时使用 `title_area 20vp + value_group(layoutWeight:1) + action_area 36vp`，标题顶部居中，`value_group` 在中部用 Column 将大数字与单位纵向居中，单位固定在数字正下方；`action_area` 必须是 root 最后一项，按钮固定 `x:12vp、y:102vp、width:126vp、height:36vp`，距卡片底部正好 `12vp`。会议时间线亚型改用 `day_area 16vp + content_area + action_area`，`day_area` 是正文日期上下文，不是 CardHeader。
- `150×150vp` 密度：不含 Progress 的普通数值亚型最多使用“大数字主值行 + 一条辅助行”，多个辅助字段合并到同一个 `12fp/400` Text；普通纯文字亚型最多使用“一条突出信息 + 两条辅助信息”。同级并行指标使用最多三条普通字号完整信息，不得选其中一项放大到 `30fp/38fp`。这些上限按标题与 action 之间的全部可见信息合计，不能通过拆成 `content_area`、`bottom_area` 等多个容器规避。
...
- region：`header_area` 固定使用 `40vp` 信息区并承担标题职责；其后间隔 `8vp`，`action_area Column -> [cta_1, cta_2]` 使用两张 `36vp` 胶囊纵排、`itemMargin:6`、总高 `78vp`；无独立 content_area。
...
2x4 每张卡必须且只能选择一个骨架，允许在声明范围内微调，不得跨骨架拼接 region。先统计数据块数量：1 个在 W1-W7 中按内容选择，2 个强制 W9，3 个强制 W10，4 个强制 W8。通用约束：独立卡级标题只允许用于单数据块骨架，有则必须使用 20vp 高的 CardHeader 作为全尺寸前景 Column 首项，无则不生成标题及空槽；W8/W9/W10 严禁卡级 title、CardHeader、公共标题行及标题空槽，并统一使用 root `padding:8`；单业务分栏仍使用各骨架原有安全区尺寸，W8/W9/W10 多业务蒙版统一宽 138vp；线性进度条一律 `strokeWidth:8`；弹性沉底用包含真实内容的 `layoutWeight:1` 容器占据剩余空间（`flexShrink` 只收缩不拉伸，禁止用于沉底）；禁止 `justifyContent:"spaceBetween"` 制造三段均分（会把中部元素推离预期位置）。

### `W1-progress-aux`（进度 + 辅助区域）

- 用于：有真实比例语义的单主指标进度 + 同业务侧栏辅助（同构事项 / 说明背板 / 双入口）；承载 1 主数值 + 1 进度 + 辅助 0-2 项。
- region：`content Row -> [progress_zone 136, aux_zone 130]` gap 10。progress_zone：`count`（默认 30fp、最大 38fp 纯数字 + 12-16fp 单位）作为第一行 → `tag 12fp/400` 放在数值下方 → 真实内容组 `layoutWeight:1` → 沉底 `Progress linear 136×8`（strokeWidth 8）+ 端标行（12fp/400/第十二节必要支撑文字色，条下方两端对齐）。aux_zone 三变体：var-a 双份区域（两张 59 高背板 + gap 8，白色 80% 底、圆角 12，各「标题 12fp/400 + 副文 12fp/400」）；var-b 整体大区域（单背板 126 全高、圆角 14，标题 400 顶部 + 正文 400 沉底、中部留白，无动作）；var-c 双按钮（两枚 130×36 胶囊纵排 gap 10，沉底与进度条底对齐）。
...
- region：`kicker 12fp/400` 顶 + `lower Column 103 justifyContent:end`：`event 66`（主内容标题最大 18fp/500 + 正文 12fp/400 两行）+ `date 12fp/400`（贴正文下方沉底）；kicker 与标题之间大留白（上空下实）。
...
- region：可选 `CardHeader 20vp` + `main Row 103`：`ringArea 134`（`Stack 92`：Progress ring strokeWidth 8 + 环心纯数字 20fp/700，未拆分数值字符串最大 18fp/700）+ `info 134`（说明标题 16fp/400 / 值 14fp/400 / 状态 12fp/400，居中）。环形 Progress 本身视为所属业务主内容第一项，除可选 CardHeader 外不得在环前增加说明或强调文字。有 CardHeader 时与 main 的间距使用 2vp，确保总高不超过 126vp。
- 槽位：一个环主指标；右侧至多三行说明。
...
- region：可选 `CardHeader 20vp` + `body 98`：`progressSlot 45`（主数值行 20fp/700 + label 12fp/400 + `Progress linear 276×8 strokeWidth 8`）+ `details Row 45`（两背板 134×2 gap 8、圆角 10、白色 80% 底，各「标题 12fp/400 + 值 12fp/400」）。
- 槽位：一个进度主指标 + 两个详情项。
...
- 用于：近期日程/待办 3 行清单。
- region：可选 `CardHeader 20vp` + `list 102`：三行背板 276×28（gap 8）、圆角 8、白色 80% 底；每行文本 12fp/400，不附加装饰图标。
- 槽位：恰好三行；每行一条文本。
- 禁止：两行或四行、行内按钮、行高不一。
...
- region：无卡级标题；root `padding:8`，使用 `284×134vp` 的 `2×2` 网格，两行两列的横纵 itemMargin 均为 8，每个小内容蒙版固定 `138×63vp`、padding 12、borderRadius 16、白色 80% 背景，并完整遵守 8.3 节小内容蒙版的两行数据、右侧图标和数据删减规则。仅在蒙版内有 1-2 项信息且需要比例视觉时允许使用右侧 `40×40vp、strokeWidth:4` 的环形 Progress 与环心 20×20vp 图标；3 项信息时不得保留环图，否则不得为填充版面强加环图。
...
- region：无卡级总标题；`root Row` 以 `width/height:matchParent` 填满 `300×150vp` 画布，padding 8、itemMargin 8，直接包含左右两个 `138×134vp` 同级背板。两背板均为 Column、padding 12、borderRadius 16、白色 80% 背板，内部可用空间均为 `114×110vp`。这是 2x4 双业务唯一合法的一级布局，不得套用 2x2 S4 的上下两个 `134×63vp` 结构，也不得改成上下两个全宽背板。
- 每侧按 2x2 的层级组织；需要突出量化主值时按「对象名 + 主读数 + 必要辅助信息 + 可选底部按钮 114×36vp」排列；有必要对象名时先说明对象，再突出主值；没有突出数字时仍可按「业务标题 12fp/400 + 主内容 + 可选底部按钮」排列。有按钮时主内容使用剩余高度，每侧最多三行视觉信息。业务标题只是背板内分区标题，不使用 CardHeader。W9 中的倒计时必须写成同一行普通主数据（如 `30天`），固定 `14fp/700`，禁止使用单业务倒计时的 `30fp/38fp` hero，也禁止把单位“天”拆成独立一行。
- 每个 W9 背板按 `150×150vp` 的 2x2 内容密度独立检查：内部 `12fp/400` 业务标题和底部 action 不计入信息行；使用 `30fp/38fp` 纯数字主值时，除主值/单位行外最多一条 `12fp/400` 辅助信息；纯文字或格式化主读数最多一条突出信息和两条 `12fp/400` 辅助信息；同级并行数据最多三条普通字号完整信息，不得制造多个突出焦点。多个辅助字段必须在所属背板内使用 ` | ` 合并，不得移动到另一背板或背板外。
- 稀疏分区：某侧最终只保留 1-2 项必要事实且无底部按钮时，内容组使用 `justifyContent:"center"`，主值优先使用安全的 20/24/30fp 档位，核心状态优先使用 16/18fp，辅助文字保持 12fp 并紧贴主内容。有语义精确且状态安全的候选素材时，优先把 `20×20vp` 业务图标放在标题右侧或主内容组右侧，文字组与图标间距固定 8vp；内容已达 3-4 行、图标会挤压必要文字或没有精确候选时保持纯文字，不留空槽。左右不要求机械对称，但应通过主字号、内容组位置和连续留白保持接近的视觉重量。
...
- 禁止：上下双业务布局、`root Stack -> content Column -> [zone_1, zone_2]`、两个全宽 `276×59vp` 背板、卡级总标题、主辅不对等分栏、背板外动作、整卡或整块背板点击、左右业务共用按钮、第三业务。
...
- region：无卡级标题；`root Row` 以 `width/height:"matchParent"` 填满 `300×150vp` 画布，padding 8、itemMargin 8，直接包含左侧 `138×134vp` 大内容蒙版和右侧 `138×134vp` Column。右侧 Column 使用 itemMargin 8，上下各放一个 `138×63vp` 小内容蒙版。三个内容蒙版均使用 padding 12、borderRadius 16、白色 80% 背景。
- 左侧大内容蒙版需要突出量化主值时按「对象名 + 主读数 + 必要辅助信息 + 可选底部 Button 114×36vp」组织；没有突出数字时仍按「业务标签 12fp/400 + 主内容 + 可选底部 Button」组织。有动作时只绑定内部按钮，大内容蒙版本身不绑定，主内容最多两行最小充分信息。右侧两个分区完整遵守 8.3 节小内容蒙版的两行数据、右侧图标和数据删减规则。
...
**格式化主读数例外（与转换前校验一致）**：适用于明确描述为温度、时长或百分比的主读数，不限于单一 data 根；名称、日期、时间、状态不适用。内容可以是直接 PathBinding，也可以是只引用一个字段并追加真实单位的简单 Expression，不得拼接名称、状态或说明。Text 必须单行、无 padding/margin、显式高度至少为字号的 1.4 倍，并与实际主内容槽同宽：2x2 使用 126vp，2x4 全宽使用 276vp，W9/W10 左侧 `138×134vp` 大背板使用内部 114vp；父级 Column/Row 必须同宽且无 padding。只允许 20fp 或 24fp；2x2 的 126vp 内容槽和 W9/W10 的 114vp 内容槽默认使用 20fp，只有更宽的内容槽通过压力预算后才可使用 24fp。须以较长合法值（负温、100%、两位数小时和分钟）加 20% 余量检查宽度。W8、小内容蒙版与环中心不使用该例外；不能通过增大 Text 声明宽度突破父级预算。
...
- `2x4` 底部动作区只有一个卡级 Button 或图文按钮 Row 时，该按钮固定为 `x:12vp、y:102vp、width:276vp、height:36vp`，距卡片底部必须正好 `12vp`；按钮必须是满高前景 Column 的最后一项，上方真实内容区使用 `layoutWeight:1` 占据剩余空间。禁止生成 `126/130/134vp` 的半宽单按钮或浮动底边距。W9 和 W10 左侧大背板内按钮是例外，固定宽 `114vp` 并留在所属业务背板内。
...
- 横向进度固定写法：`Progress type:"linear"`，2x2 宽 126、高 8、圆角 4，写 `value`、`total`、`color`、`backgroundColor`；颜色基准跟随本卡内容色，轨道使用内容色 20%，不再使用旧色值或黑灰色。不要生成 4vp 细进度条。
...
- 环形进度固定写法：`Progress type:"ring"`，2x2 单业务的 Stack 与 Progress 只能是 `48×48vp`，S4 双业务每区的 Stack 与 Progress 只能是 `44×44vp`，两者的 `strokeWidth` 都只能是 `6vp`，禁止使用其它数值。环内语义 Image 在 `2x2` 固定 `20×20vp`，`2x4` 保持原骨架尺寸；环内需要百分比读数时使用 Text，或使用只含数值 Text 与单位 Text 的紧凑 Row；小数、长字符串和多行说明必须放在环外。
...
目标是轻盈、有层次、有一个清楚的视觉焦点。采用 UX 的同色相微渐变、白色内容背板和少量彩色素材，不用提高整卡饱和度制造吸引力。未指定配色时，root 背景收敛为以下蓝、紫、暖三套浅色渐变与内容色，严格采用 UX「纯色渐变背景」详细规范，不自行调深、调浅或派生替代色。青、绿、粉红可作为合法多色素材、图表或真实状态的局部色彩，但不作为默认 root 背景。
...
4. 色板由核心业务确定，不按 few-shot 的主题机械复制。天气、耳机、电量、设备、未知业务默认蓝色；睡眠、专注、一般运动和健康概览默认紫色；日程、会议、倒计时和纪念日默认暖色。心率主读数使用紫色 root，只在合法图表、图标或已由动态数据证明的局部状态中使用粉红；不随预览值擅自切换告警色。多业务以用户明确的主业务为准，无主次或未覆盖时用蓝色。
...
- 彩色素材是局部视觉锚点，不是每行配图的理由；新增 PNG 仅按精确 description 匹配单业务主题，并按上面的 CardHeader 例外使用。没有合法彩色素材时使用合法单色素材或纯文字，不生成 Unicode emoji、不猜测路径、不拉伸普通图标为背景。
...
1. **输出与协议**：是否只有一个 `genui` 代码块和可解析的极简协议 JSONL；是否没有 createSurface/updateComponents/updateDataModel/surfaceId/catalogId；root、组件字段和枚举是否正确；用户未指定配色时 root 是否只使用蓝/紫/暖微渐变或合法融球；融球是否满足第十二节的 2x2、单业务、最终信息项不超过 3、单内容组、显式动作不超过 1 和运行时条件，且 root 未同时写普通背景。
...
4. **骨架与预算**：是否保留每个并列对象的名称和有歧义指标的标签；必要文字是否使用主文字色而非 60%；普通 S2 是否主信息顶对齐、描述紧跟、动作沉底；是否只使用一个固定骨架；单业务双按钮是否为 S3 的 40vp 信息组加 78vp 动作组，主信息是否合并为 14fp 且没有独立标题和大数字内容区；是否按最终展示对象正确统计数据块，2x4 的 1/2/3/4 个数据块是否分别路由到 W1-W7/W9/W10/W8，2x2 恰好两个数据块是否强制使用完整双分区 S4，单数据块是否未生成单个 `134×63vp` 内容背板或残缺 S4，且没有把任一数据块降级为另一数据块的辅助信息；2x2 S4 两个背板内的文字、可染色图标、Progress 和 Divider 是否统一使用同一主内容色 RGB；所有小内容蒙版是否只有两行、左文字右 visual，1-2 项如有右侧图标，是否语义准确且状态安全，3 项时是否去掉 visual 并以 ` | ` 合并辅助数据，超过 3 项是否删除低优先级项，图标或环外框是否距蒙版右边 12vp；CardHeader 是否只用于 2x2 S2 或 2x4 单数据块的独立卡级标题、每卡最多一个且位于规定前景 Column 首项，W8/W9/W10 是否没有卡级标题、CardHeader、公共标题行或标题空槽，无标题布局是否未留空槽，S1/S3/S4 是否未加 CardHeader 或右上角辅助图标；root 宽高是否为 `"matchParent"`、普通单业务布局 padding 12、S4 及 W8/W9/W10 padding 8、圆角 20、clip true；root Column 是否使用 `alignItems:"center"`，root Row 的固定参考宽度直接内容是否使用 `justifyContent:"center"`，且没有把 root 的整组居中误写成内部文字全部居中；是否已按 3.1 节逐项写出 root 直接子节点的最小高度、margin 和 `itemMargin` 并确认普通布局 `H_required <= 126vp`、S4 及 W8/W9/W10 `H_required <= 134vp`；分布式对齐是否只分配扣除最小间距后的非负剩余空间；所有 Row/Column 两轴预算是否非负，动态文字 Row 是否保留余量，点击热区是否至少 24vp。
...
6. **表面与素材**：逐个核对 Image 的实际用途，2x2 普通内容区必须为零，例外仅为环中心或 S4 分区主视觉；S4 每区最多 1 个，两个天气对象有语义准确且状态安全的候选素材时是否各有 1 个，重复 src 是否分别计数，辅助文字前后是否无图标；W9 稀疏分区有精确候选素材时是否优先使用右侧业务图标，密集分区是否未为对称强塞图标；是否按最终图标实例计数、默认不超过 2 个且按 2.5 节预先分配；2x2/2x4 的所有图标是否均为 20×20vp 且没有主辅尺寸分档；是否仅为用户明确要求放宽数量，且未丢失动作或预留空图标槽；未有用户明确要求时，是否只使用蓝、紫、暖三套固定浅色微渐变或五套融球；业务映射和融球优先级是否正确；文字与单色图标、按钮、背板是否遵循本卡固定配色和透明度；进度与分隔线是否只更新颜色基准而保留原样式；是否保留多色图标与真实状态色例外，且没有旧的同色背板、特殊深色或自由取色残留。
...
**2x2 倒计时扩展布局约束**：仅有倒计时名称、数字和单位且没有动作或其它可见数据时使用 V01 居中布局；只要存在按钮、开始时间、日期或状态等额外信息，必须切换 V08。V08 的标题、`value_group` 和辅助信息左对齐，数字与“天”位于同一 `value_row`，辅助信息最多一行，动作固定沉底，禁止把数字、单位和辅助信息拆成三行。

**指标语义硬门禁**：指数、等级、评分、得分、概率、风险、质量和健康类短值必须在同一文本或相邻文本中带明确指标名，例如“感冒指数 中等”“睡眠得分 82”；卡片标题不能替代多个并列指标各自的标签。

**融球组合简化约束**：2x2 融球卡不得同时组合标题/辅助图标、环形 Progress、多个状态文本和按钮；保留一个主视觉焦点，必要时合并状态文字、移除图标或回退普通布局。
